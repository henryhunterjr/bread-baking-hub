    ...
    try {
      ...
      const payload = {
        postData: { 
          ...
          slug: postData.slug || '', // Will be generated by backend if empty
          hero_image_url: postData.heroImageUrl || null,
          inline_image_url: postData.inlineImageUrl || null,
          social_image_url: postData.socialImageUrl || null,
          tags: postData.tags || [],
      ...
      const heroImageUrl = result.hero_image_url;
      
      if (!publishedSlug) {
        console.error('No slug returned from published post');
        toast({
          title: "Published with warning",
          description: "Post is live but URL generation failed. Check your posts list.",
          variant: "destructive"
        });
        return;
      }
      ...
      if (postData.publishAsNewsletter) {
        ...
        const postUrl = `${window.location.origin}/blog/${publishedSlug}`;
        toast({
          title: "Blog post live",
          description: (
            <div className="space-y-2">
              <p>Your blog post has been published successfully!</p>
              <p className="text-sm text-muted-foreground">Slug: {publishedSlug}</p>
              {heroImageUrl && <p className="text-sm text-muted-foreground">Hero image: ✓</p>}
              <Button 
                variant="outline" 
                size="sm" 
