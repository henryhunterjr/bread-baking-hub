...
// Lazy load blog card outside render loop
const BlogCard = React.lazy(() => Promise.resolve({
  default: ({ post, enableSEO, handlePostClick, categories }: {
    post: BlogPost;
    enableSEO?: boolean;
...
const BlogSkeletonGrid = ({ count }: { count: number }) => (
...
const BlogPostGrid = ({ posts, loading, skeletonCount = 6, selectedCategory, categories = [], enableSEO = false }: BlogPostGridProps) => {
  ...
  if (loading) {
    return <BlogSkeletonGrid count={skeletonCount} />;
  }
