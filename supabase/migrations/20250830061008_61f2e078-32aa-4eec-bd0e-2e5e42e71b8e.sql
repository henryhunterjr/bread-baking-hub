-- Security Migration: Fix remaining functions (with proper drops)
-- Address the remaining function search path warnings

-- Drop and recreate functions that exist with different signatures
DROP FUNCTION IF EXISTS public.apply_security_hardening();
DROP FUNCTION IF EXISTS public.assign_ab_variant(text, uuid, text);
DROP FUNCTION IF EXISTS public.create_recipe_version(uuid, text);
DROP FUNCTION IF EXISTS public.get_core_web_vitals_summary(integer);
DROP FUNCTION IF EXISTS public.get_related_recipes(uuid, integer);
DROP FUNCTION IF EXISTS public.get_trending_recipes(integer, integer);
