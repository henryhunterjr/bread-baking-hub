-- Phase B: Proper Data Model with namespaced tables and materialized views

-- First, migrate existing data to new table structure
...
CREATE UNIQUE INDEX idx_app_analytics_events_event_id_recent 
    ON public.app_analytics_events (event_id) 
...
-- Materialized View: Subscribers per Day (using existing newsletter_subscribers if available)
...
AS $$
BEGIN
    REFRESH MATERIALIZED VIEW CONCURRENTLY public.app_analytics_mv_sessions_by_source_day;
    REFRESH MATERIALIZED VIEW CONCURRENTLY public.app_analytics_mv_page_perf_day;
    REFRESH MATERIALIZED VIEW CONCURRENTLY public.app_analytics_mv_subscribers_day;
    REFRESH MATERIALIZED VIEW CONCURRENTLY public.app_analytics_mv_errors_by_route_day;
    REFRESH MATERIALIZED VIEW CONCURRENTLY public.app_analytics_mv_cwv_by_page_day;
