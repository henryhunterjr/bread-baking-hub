-- Security Migration: Fix Function Search Paths and Extensions (Fixed)
...
BEGIN
  ...
  SELECT 
    ...
    bp.title,
    bp.slug,
    SUBSTRING(bp.content, 1, 200) as excerpt, -- Create excerpt from content
    bp.hero_image_url,
    bp.tags,
    (
      -- Title matches get highest score
...
SELECT 
  ...
  title,
  slug,
  SUBSTRING(content, 1, 200) as excerpt, -- Create excerpt from content
  content,
  hero_image_url,
  inline_image_url,
  social_image_url,
...
-- Create RLS policy to deny direct access to blog_posts for anon users
-- Force them to use the secure view instead
DROP POLICY IF EXISTS "Block anon direct access to blog_posts" ON public.blog_posts;
CREATE POLICY "Block anon direct access to blog_posts" 
